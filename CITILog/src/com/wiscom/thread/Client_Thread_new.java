...
public class Client_Thread_new extends Thread {
	private static Logger logger = Logger.getLogger(Client_Thread_new.class);
	...
	public String alertFormat = "摄像机编号：%s，对应的ID： %s，告警状态：%s，事件开始时间：%s，事件结束时间：%s，车道类型：%s，事件类型：%s，采集时间：%s";
	public String RemoveFormat = "摄像机编号：%s，对应的ID：%s，告警状态：%s，事件开始时间：%s，事件结束时间：%s，车道类型：%s，事件类型：%s，采集时间：%s";
				...
				while (true) {
					...
					try {
						...
						in.read(receive);
							...
							String event = new String(result, "UTF-8").trim();
							logger.info(event);
	...
	protected void handleTask(String event) {
		...
		switch (type) {
		case "2":
//			for (Map.Entry<String, String> entry : VariablesMain.confMap.entrySet()) {
//				if (entry.getValue() == split[2]) {
//					key = entry.getKey() + split[4] + split[3];
//					VariablesMain.alertMap.put(key, new AlertDetail(entry.getKey(), convertType(split[4]),
//							Integer.parseInt(split[3]), 0, "N", startTime, null, dateFormat.format(new Date())));
//				}
//			}
			if(VariablesMain.confMap.containsKey(split[2])) {
				VariablesMain.alertMap.put(key, new AlertDetail(VariablesMain.confMap.get(split[2]).split(",")[0], convertType(split[4]),
						Integer.parseInt(split[3]), 0, "N", startTime, null, dateFormat.format(new Date()), VariablesMain.confMap.get(split[2]).split(",")[1]));
			}
			break;
		case "3":
			if(split.length<9) {
			...
			if (VariablesMain.confMap.containsKey(split[2])) {
					VariablesMain.alertMap.put(key, new AlertDetail(VariablesMain.confMap.get(split[2]).split(","
							+ "")[0], convertType(split[4]),
							Integer.parseInt(split[3]), 1, "N", startTime, endTime, dateFormat.format(new Date()), VariablesMain.confMap.get(split[2]).split(",")[1]));
			}
			break;
		default:
			break;
					...
					try {
						...
						for (Map.Entry<String, AlertDetail> entry : VariablesMain.alertMap.entrySet()) {
							...
							if (entry.getValue().getAlertSta() == 0) {
								logger.info(String.format(alertFormat, entry.getKey().split("-")[0], entry.getValue().getCamId(),
										entry.getValue().getAlertSta(), entry.getValue().getStartTime(), "", entry.getValue().getLanNo(),
										entry.getValue().getTypeId(), entry.getValue().getCollectedTime()));
								logger.info("insert alert event successfully");
								System.out.println();
							}
							if (entry.getValue().getAlertSta() == 1) {
								logger.info(String.format(RemoveFormat, entry.getKey().split("-")[0], entry.getValue().getCamId(),
										entry.getValue().getAlertSta(), entry.getValue().getStartTime(), entry.getValue().getEndTime(),
										entry.getValue().getLanNo(), entry.getValue().getTypeId(), entry.getValue().getCollectedTime()));
			...
			public void run() {
				while (true) {
					...
					try {
						...
						logger.error(e.getMessage());
						closeSocket();
						logger.error("server closes the connection...waiting for reconnecting again and send data");
						try {
							Thread.sleep(1000);
							initSocket();
